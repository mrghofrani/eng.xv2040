...
// Wait for a child process to exit and return its pid.
// Return -1 if this process has no children.
// Difference is using this function parent could know creationTime, terminationTime, etc from child process
int
waitForChild(struct timeVariable* tv)
{
    struct proc *p;
    int havekids, pid;
    struct proc *curproc = myproc();
